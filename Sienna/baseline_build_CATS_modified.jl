...
function build_CATS_system(;
    ...
    for (col_name, col_info) in col_to_type_and_kwargs
        ...
        # Add time series only to non-Shasta generators for Large Hydro, since Shasta has its own time series.
        filtered_comps = [comp for comp in comps if get_name(comp) ∉ shasta_gen_names]
        associations = (TimeSeriesAssociation(comp, ts) for comp in filtered_comps)
    ...
    for (plant_name, (filename, gen_names)) in selected_hydro_details
        ...
        @assert length(p_max_values) == length(timestamps) "Expected 8760 rows of 2019 hydro data, found $(length(p_max_values))."
        ...
        for comp in selected_comps
            ...
            # Write unit_min_normalized to a CSV file for debugging purposes
            #debug_csv_path = joinpath(BASE_DIR, "Script", "$(get_name(comp))_min_active_power.csv")
        ...
        #=  
        for comp in selected_comps
            if !has_time_series(comp, SingleTimeSeries, "min_active_power")
                println("WARNING: $(get_name(comp)) missing min_active_power time series")
            else
                println("✓ $(get_name(comp)) has min_active_power")
            end
        end 
        =#
