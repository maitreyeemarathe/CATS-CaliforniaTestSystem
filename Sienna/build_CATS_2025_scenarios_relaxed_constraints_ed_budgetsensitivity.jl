...
attach_cost!(gen::ThermalStandard, cost::CostCurve; startup=0.0, shutdn=0.0) =
    set_operation_cost!(gen, ThermalGenerationCost(cost, 0.0, startup, shutdn))
...
"""
Return generator names (e.g., "gen-12") for hydro units matching:
- plant_code
- bus number
- optional GenID list

Use expected_count to enforce data consistency.
"""
function get_hydro_gen_names(
    gen_csv::DataFrame;
    plant_code::Int,
    bus::Int,
    gen_ids::Union{Nothing, AbstractVector{<:AbstractString}} = nothing,
    expected_count::Union{Nothing, Int} = nothing,
)
    # Required columns in CATS_gens.csv
    required_cols = (:PlantCode, :bus, :GenID)
    @assert all(c -> c in propertynames(gen_csv), required_cols) "generator_csv missing one of required columns $(required_cols)"

    # Optional filter for specific GenID values
    genid_set = isnothing(gen_ids) ? nothing : Set(string.(gen_ids))

    # Build matching internal generator names: gen-<row_index>
    gen_names = Set{String}()
    for (i, row) in enumerate(eachrow(gen_csv))
        same_plant_bus = (row[:PlantCode] == plant_code) && (row[:bus] == bus)
        same_genid = isnothing(genid_set) || (string(row[:GenID]) in genid_set)

        if same_plant_bus && same_genid
            push!(gen_names, "gen-$(i)")
        end
    end

    # Optional sanity check on count
    if !isnothing(expected_count)
        @assert length(gen_names) == expected_count "Expected $(expected_count) generators for plant=$(plant_code), bus=$(bus), found $(length(gen_names))."
    end
...
function build_CATS_system(;
    ...
    timeseries_csv::String = joinpath(DATA_DIR, "HourlyProduction2025.csv"),
    load_timeseries::String = joinpath(DATA_DIR, "Load_Agg_2025.csv"),
    remove_scs::Bool = true,
    fraction_reduction::Float64 = 0.20,
    start_time::DateTime = DateTime("2025-01-01T00:00:00"),
    pmin_scale,
)
    ...
    for (i, row) in enumerate(eachrow(gen_csv))
        ...
        if VALIDITY_CHECKS
            ...
            if !(comp isa SynchronousCondenser || comp isa EnergyReservoirStorage || comp isa FixedAdmittance)
                ...
                # Pmin intentionally differs: MATPOWER modified with random values, CSV unchanged
            end
    ...
    # Find all generators that are ThermalStandard and have fuel type MUNICIPAL_WASTE
    for comp in get_components(ThermalStandard, system)
        get_fuel(comp) == ThermalFuels.MUNICIPAL_WASTE || continue
        limits = get_active_power_limits(comp)
        set_active_power_limits!(
            comp,
            (min = limits.min, max = 3.0 * limits.max),
        )
    end
    ...
    ts_df = CSV.read(timeseries_csv, DataFrame;
        header=1,
        # 2019 file leads with Date/Hour/Interval, 2025 file leads with a single Time column
        types=(i, name) -> String(name) in ("Date", "Hour", "Interval", "Time") ? String : Float64,
    )
    
    shasta_csv = joinpath(HYDRO_DATA_DIR, "shasta_$(SCENARIO)_$(YEAR)_hourly.csv")
    devilcanyon_csv = joinpath(HYDRO_DATA_DIR, "devilcanyon_$(SCENARIO)_$(YEAR)_hourly.csv")
    mammoth_csv = joinpath(HYDRO_DATA_DIR, "mammoth_$(SCENARIO)_$(YEAR)_hourly.csv")
    ...
    timestamps = range(DateTime("2025-01-01T00:00:00"); step = Hour(1), length = 24*365)
    ...
    for (col_name, col_info) in col_to_type_and_kwargs
        ...
        ###### Co-pilot generated #########
        if col_name == "Large Hydro"
            comps = [comp for comp in comps if get_name(comp) ∉ selected_gen_names]
        end
        #################
        ...
        if col_name == "Nuclear"
            # add a minimum for nuclear
            min_ratio_nuclear = 0.7
            for comp in comps
                set_active_power_limits!(
                    comp,
                    (min = get_active_power_limits(comp).max * min_ratio_nuclear, 
                    max = get_active_power_limits(comp).max)
                )
            end
        elseif col_name == "Thermal"
            # For fuel type 
            for comp in comps
                set_active_power_limits!(
                    comp,
                    (min = get_active_power_limits(comp).max * 0.001, 
                    max = get_active_power_limits(comp).max)
                )
            end
        end
        ...
        if any(ts_values .> 1.0)
            ...
            ts_values ./= scale_up
            for comp in comps            
                if comp_type == RenewableDispatch
                    set_rating!(comp, get_rating(comp) * scale_up)
                else
                    set_active_power_limits!(
                        comp,
    ...
    for (plant_name, (filename, gen_names)) in selected_hydro_details
        @assert isfile(filename) "Missing hydro time series file: $filename"
        hourly_df = CSV.read(filename, DataFrame)
        @assert all(col -> col in propertynames(hourly_df), (:datetime, :week_p_max, :week_p_min)) "Hydro data must contain datetime, week_p_max, and week_p_min columns"
        #hourly_df = transform(hourly_df, :datetime => ByRow(Date) => :_datetime)
        #sort!(hourly_df, :_datetime)
        #hourly_df = filter(row -> year(row[:_datetime]) == 2019, hourly_df)
        p_max_values = convert(Vector{Float64}, collect(hourly_df[!, :week_p_max]))
        p_min_values = convert(Vector{Float64}, collect(hourly_df[!, :week_p_min]))
        @assert length(p_max_values) == length(timestamps) "Expected 8760 rows of 2025 hydro data, found $(length(p_max_values))."

        selected_comps = StaticInjection[]
        for gen_name in gen_names
            comp = get_component(StaticInjection, system, gen_name)
            @assert !isnothing(comp) "Could not find hydro generator $gen_name"
            @assert comp isa HydroDispatch "Expected $gen_name to be HydroDispatch after type conversion"
            push!(selected_comps, comp)
        end

        for comp in selected_comps
            unit_max = get_max_active_power(comp)
            unit_share = unit_max / sum(get_max_active_power(c) for c in selected_comps)

            unit_max_hourly = p_max_values .* unit_share
            unit_max_normalized = unit_max_hourly ./ unit_max
            ts_max = SingleTimeSeries(;
                name = "max_active_power",
                data = TimeArray(timestamps, unit_max_normalized),
                scaling_factor_multiplier = get_max_active_power,
            )
            bulk_add_time_series!(system, [TimeSeriesAssociation(comp, ts_max)])
            unit_min_hourly = p_min_values .* unit_share
            unit_min_normalized = unit_min_hourly ./ unit_max
            unit_min_normalized = pmin_scale*unit_min_normalized # scale down min to 25% of original
    ...
    elseif isfile(load_timeseries)
        ...
        @warn("Converting CSV to JLD2 first using convert_load_csv_to_jld2.jl." *
            ...
            include(joinpath(CATS_DIR,"convert_load_csv_to_jld2_2025.jl"))
    end
    load_data = load(jld2_file, "load_data")

    # Adjust each modeled week separately, preserving its total active load.
    @assert HORIZON_WEEKS > 0 "load_skew_horizon_weeks must be positive"
    horizon_end = min(start_time + Week(HORIZON_WEEKS), last(timestamps) + Hour(1))
    for week_start in start_time:Week(1):(horizon_end - Week(1))
        week_end = min(week_start + Week(1), horizon_end)
        ...
        weekday_active_total = sum(real.(load_data[weekday_rows, :]))
        weekend_active_total = sum(real.(load_data[weekend_rows, :]))
        weekday_scale = (weekday_active_total + weekend_active_total * fraction_reduction) / weekday_active_total
        load_data[weekday_rows, :] = complex.(real.(load_data[weekday_rows, :]) .* weekday_scale, imag.(load_data[weekday_rows, :]))
        load_data[weekend_rows, :] = complex.(real.(load_data[weekend_rows, :]) .* (1.0 - fraction_reduction), imag.(load_data[weekend_rows, :]))
    end
    ...
    if VALIDITY_CHECKS
        ...
        # Removing this check for now because load is being scaled for Oct 21-25 and redistributed to Oct 26-27, so the first row of the CSV will not match the first column of load_data after scaling.
    ...
    for (i, row) in enumerate(eachrow(cost_df))
        ...
        if comp isa ThermalStandard
            row[:startup] = 5000 + 5000*rand()
        end
        n = row[:n]
        ...
        c1 = row[:c1]
        c0 = row[:c0]
        # Renewables always use a fixed slope regardless of MATPOWER cost data
        # (MATPOWER typically stores 0 for renewables, which would trigger the
        # all-zero guard below and leave the cost as `nothing`).
        if comp isa RenewableDispatch
            attach_cost!(comp, CostCurve(LinearCurve(-5, 0)))
            continue
        end

        if comp isa ThermalStandard && get_fuel(comp) == ThermalFuels.NUCLEAR
            # https://www.eia.gov/outlooks/aeo/assumptions/pdf/elec_cost_perf.pdf
            attach_cost!(comp, CostCurve(LinearCurve(3, 0)))
            continue
        end
        ...
        elseif comp isa HydroDispatch
            ...
            cost_curve = CostCurve(function_data)
            #attach_cost!(comp, cost_curve)
            attach_cost!(comp, cost_curve; startup=row[:startup], shutdn=row[:shutdown])
        end
    end
