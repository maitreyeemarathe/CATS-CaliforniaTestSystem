...
RESULTS_DIR = "$BASE_DIR/results/annual_baseline_uc/"
...
#    "log_to_console" => true, "mip_rel_gap" => 0.05)
...
vuc = read_variables(uc_results)
duc = read_duals(uc_results)

# Write dispatch of all generators to XLSX for external analysis. Make a new sheet for each timestep
dispatch_hydro = read_variable(uc_results, "ActivePowerVariable__HydroDispatch")
dispatch_thermal = read_variable(uc_results, "ActivePowerVariable__ThermalStandard")
dispatch_renewable = read_variable(uc_results, "ActivePowerVariable__RenewableDispatch")
...
hydro_key = only([k for k in keys(vuc) if occursin("ActivePowerVariable", String(k)) && occursin("HydroDispatch", String(k))])
price_key = only([k for k in keys(duc) if occursin("CopperPlateBalanceConstraint", String(k))])


hydro_sd = vuc[hydro_key]   # SortedDict{DateTime, DataFrame}
price_sd = duc[price_key]   # SortedDict{DateTime, DataFrame}
...
CSV.write(joinpath(RESULTS_DIR, "hydro_uc_revenue_by_generator.csv"), by_gen)
CSV.write(joinpath(RESULTS_DIR, "hydro_uc_revenue_by_plant.csv"), by_plant) 
...
CSV.write(joinpath(RESULTS_DIR, "hydro_uc_prices.csv"), price_usd_per_mwh_df)  # convert from $/100 MWh to $/MWh

# Get maximum dispatch bounds for hydro plants
hydro_max_param_dict = read_parameter(uc_results, "ActivePowerTimeSeriesParameter__HydroDispatch")
hydro_min_param_dict = read_parameter(uc_results, "MinActivePowerTimeSeriesParameter__HydroDispatch")
