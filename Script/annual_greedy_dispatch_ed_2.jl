...
function generate_offer_curve(
    ...
    forecast_perfect::Bool = true,
    day::Int = 1
)
    if day == 1 || forecast_perfect
        price_forecast_file = joinpath(HYDRO_DATA_DIR, "baseline_dailybudget_forecast.csv")
        ...
        price_forecast_file = joinpath(RESULTS_DIR, "hydro_ed_prices.csv")
        price_forecast_df = read_price_forecast(price_forecast_file, start_time)
        price_forecast_df = filter(
            row -> row.DateTime >= start_time - optimization_horizon_hours && row.DateTime < start_time,
            price_forecast_df,
        )
    end
