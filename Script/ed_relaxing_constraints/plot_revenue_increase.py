...
Each results/ed_relaxing_constraints/rcp45hotter/<start_date>/ folder holds a
...
RESULTS_DIR = os.path.join(BASE_DIR, "results", "ed_relaxing_constraints", "rcp45hotter")
...
def weekly_revenue_by_plant(run_dir, plant_gen_names):
    ...
    price = pd.read_csv(
        os.path.join(run_dir, "shadow_prices.csv"), parse_dates=["DateTime"]
    )
