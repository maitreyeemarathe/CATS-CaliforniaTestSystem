...
HYDRO_DATA_DIR = "$BASE_DIR/hydro_data/historical/"
CATS_DIR = "$BASE_DIR/Sienna/"
gen_csv = CSV.read("$BASE_DIR/GIS/CATS_gens.csv", DataFrame)

include(joinpath(CATS_DIR, "build_CATS_2025_scenarios_relaxed_constraints_ed_budgetsensitivity.jl"))
...
results_dir = "$BASE_DIR/results/budget_sensitivity/$(Dates.format(start_time, "yyyy-mm-dd"))/"
equal_results_dir = "$results_dir/equal/"
greedy_results_dir = "$results_dir/greedy/"

bud_system = build_CATS_system(; fraction_reduction = 0.0, start_time = start_time, pmin_scale = 1.0)

bud_tag     = "bud$(YEAR)"
equal_dir  = joinpath(results_dir, "equal_$bud_tag")
greedy_dir = joinpath(results_dir, "greedy_$bud_tag")
mkpath(equal_dir)
mkpath(greedy_dir)

run_equal_daily_budget(template, deepcopy(bud_system), solver_xpress, start_time, equal_dir, selected_gen_names, selected_hydro_details)

run_greedy_daily_budget(deepcopy(bud_system), selected_gen_names, selected_hydro_details, start_time, solver_xpress, equal_dir, greedy_dir)
