...
RESULTS_DIR = "$BASE_DIR/results/annual_uc/"
CATS_DIR = "$BASE_DIR/Sienna/"

include(joinpath(CATS_DIR, "build_CATS_modified.jl"))
include(joinpath(dirname(@__FILE__), "annual_greedy_dispatch_uc.jl"))
...
solver_highs = JuMP.optimizer_with_attributes(HiGHS.Optimizer, 
    ...
    # Write all iterations to one CSV: first iteration creates file, later iterations append rows.
    hydro_dispatch_output_path = joinpath(RESULTS_DIR, "selected_hydro_dispatch_wide.csv")
    if i == 1
        CSV.write(hydro_dispatch_output_path, hydro_dispatch_wide)
    else
        CSV.write(hydro_dispatch_output_path, hydro_dispatch_wide; append = true, writeheader = false)
    end
    ...
    hydro_budget_output_path = joinpath(RESULTS_DIR, "hydro_budget_usage_pct.csv")
    if i == 1
        CSV.write(hydro_budget_output_path, budget_usage_df)
    else
        CSV.write(hydro_budget_output_path, budget_usage_df; append = true, writeheader = false)
    end
